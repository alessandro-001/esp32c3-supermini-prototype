...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .page-header .label { font-family: var(--mono); font-size: 0.65rem; color: var(--accent); letter-spacing: 0.2em; text-transform: uppercase; margin-bottom: 4px; }
    .page-header h1 { font-size: 1.5rem; font-weight: 700; letter-spacing: -0.02em; }
    .page-header h1 span { color: var(--accent); }
    .card { background: var(--surface); border: 1px solid var(--border); border-radius: 10px; padding: 18px; margin-bottom: 16px; transition: border-color 0.2s; }
    .card-header { display: flex; align-items: center; gap: 10px; margin-bottom: 14px; }
    .card-icon { font-size: 1.3rem; line-height: 1; }
    .card-title { font-size: 0.8rem; font-weight: 700; letter-spacing: 0.1em; text-transform: uppercase; color: var(--muted); font-family: var(--mono); }
    .row { display: flex; justify-content: space-between; align-items: center; padding: 9px 0; border-bottom: 1px solid var(--border); font-size: 0.9rem; }
    ...
    .sensor-tile { background: var(--bg); border: 1px solid var(--border); border-radius: 7px; padding: 10px 12px; }
    .sensor-tile.full { grid-column: 1 / -1; }
    .sensor-tile-label { font-family: var(--mono); font-size: 0.62rem; color: var(--muted); text-transform: uppercase; letter-spacing: 0.08em; margin-bottom: 3px; }
    .sensor-tile-val { font-family: var(--mono); font-size: 1rem; font-weight: 600; color: var(--text); }
    ...
    button { width: 100%; padding: 12px; margin-top: 12px; border: none; border-radius: 7px; font-family: var(--sans); font-size: 0.95rem; font-weight: 700; cursor: pointer; letter-spacing: 0.02em; transition: opacity 0.15s, transform 0.1s; }
    ...
    button.btn-reset   { background: rgba(255,23,68,0.1);  color: var(--red);   border: 1px solid rgba(255,23,68,0.3); }
    label { display: block; font-family: var(--mono); font-size: 0.68rem; color: var(--muted); text-transform: uppercase; letter-spacing: 0.1em; margin: 12px 0 5px; }
    input { width: 100%; padding: 10px 12px; background: var(--bg); border: 1px solid var(--border); border-radius: 7px; color: var(--text); font-family: var(--mono); font-size: 0.95rem; outline: none; transition: border-color 0.2s; }
    input:focus { border-color: var(--accent); }
    .net { padding: 10px 12px; margin: 5px 0; background: var(--bg); border: 1px solid var(--border); border-radius: 7px; cursor: pointer; font-family: var(--mono); font-size: 0.85rem; word-break: break-all; transition: border-color 0.15s; }
    .net:hover { border-color: var(--accent); color: var(--accent); }
    .hidden { display: none !important; }
    .thresh-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 8px; }
    .badge { font-family: var(--mono); font-size: 0.7rem; padding: 3px 10px; border-radius: 20px; border: 1px solid var(--border); }
    .badge.online  { background: rgba(0,230,118,0.1);  color: var(--green);  border-color: rgba(0,230,118,0.3); }
    .badge.offline { background: rgba(255,23,68,0.1);  color: var(--red);    border-color: rgba(255,23,68,0.3); }
    .badge.waiting { background: rgba(255,215,64,0.1); color: var(--yellow); border-color: rgba(255,215,64,0.3); }
    #popup-msg { position: fixed; top: -60px; left: 0; width: 100%; z-index: 9999; text-align: center; padding: 15px 0; font-family: var(--mono); font-size: 0.9rem; font-weight: 600; transition: top 0.4s cubic-bezier(.4,2,.6,1); border-bottom: 2px solid var(--accent); background: var(--surface); color: var(--text); }
    ...
    @media (max-width: 420px) { .sensor-grid, .thresh-grid { grid-template-columns: 1fr; } .sensor-tile.full { grid-column: 1; } }
  </style>
</head>
<body>
...
<div class="page">
  ...
  <!-- Device Info — live status, no register button -->
...
function showProvisionedOverlay(ip, mdns) {
  ...
  overlay.style.cssText = 'position:fixed;inset:0;z-index:99999;background:rgba(10,12,15,0.97);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:32px;text-align:center;animation:fadeIn 0.4s ease;';
  overlay.innerHTML = `
    ...
    <a href="http://${ip}" target="_blank" style="display:block;width:100%;max-width:320px;background:var(--accent);color:#000;font-weight:700;font-size:1rem;padding:14px;border-radius:8px;text-decoration:none;margin-bottom:12px;">Open Device UI →</a>
...
function updateStatus() {
  ...
  fetch('/device_info').then(r => r.json()).then(info => {
    ...
    rssiEl.style.color = rssi >= -60 ? '#00e676' : rssi >= -75 ? '#ffd740' : '#ff1744';
...
<head>
  ...
  <style>
    :root{--bg:#0a0c0f;--surface:#111418;--border:#1e2530;--accent:#00e5ff;--green:#00e676;--yellow:#ffd740;--red:#ff1744;--muted:#4a5568;--text:#e2e8f0;--mono:'Courier New','Lucida Console',monospace;--sans:'Trebuchet MS','Segoe UI',sans-serif;}
    *{box-sizing:border-box;margin:0;padding:0;}
...
<div class="wrap">
  <header>
    ...
    <div class="status-pill" id="status-pill"><span class="dot"></span><span id="status-text">Ready</span></div>
  </header>
  <div class="controls">
    <div class="field"><label>Subnet prefix</label><input type="text" id="subnet" value="192.168.0" placeholder="e.g. 192.168.0"></div>
    <div class="field" style="max-width:100px;"><label>From</label><input type="number" id="range-from" value="1" min="1" max="254"></div>
    <div class="field" style="max-width:100px;"><label>To</label><input type="number" id="range-to" value="254" min="1" max="254"></div>
    <div class="field" style="max-width:100px;"><label>Timeout (ms)</label><input type="number" id="timeout" value="800" min="200" max="3000" step="100"></div>
  ...
  <div class="grid" id="grid">
    <div class="empty"><div class="icon">📡</div><p>Enter your subnet and hit <strong>Scan Network</strong>.<br>All ESP32 Smart Monitor units on the same WiFi will appear here.</p></div>
  </div>
</div>
<script>
let scanning=false,stopFlag=false,foundCount=0,scanStart=0,statsTimer=null;
function setStatus(text,state){const pill=document.getElementById('status-pill'),txt=document.getElementById('status-text');pill.className='status-pill '+(state||'');txt.textContent=text;}
...
function renderCard(ip,info,sensors){
  ...
  const sensorsHtml=sensorItems.length
    ?sensorItems.map((s,i)=>{const fw=s.label.includes('Air Status')||(sensorItems.length%2!==0&&i===sensorItems.length-1);return '<div class="sensor-item'+(fw?' full-width':'')+'"><div class="sensor-label">'+s.label+'</div><div class="sensor-val '+s.cls+'">'+s.val+'</div></div>';}).join('')
...
async function fetchWithTimeout(url,ms){const ctrl=new AbortController(),tid=setTimeout(()=>ctrl.abort(),ms);try{const r=await fetch(url,{signal:ctrl.signal});clearTimeout(tid);return r;}catch{clearTimeout(tid);return null;}}
async function probeIp(ip,timeoutMs){
  ...
  let info={};try{info=await infoRes.json();}catch{return null;}
  if(!info.device_name||!info.device_name.startsWith('ESP32'))return null;
  let sensors={};const sensRes=await fetchWithTimeout('http://'+ip+'/sensors',timeoutMs);
...
async function startScan(){
  ...
  for(let b=0;b<ips.length&&!stopFlag;b+=BATCH){
    ...
    for(const result of results){scanned++;if(result){foundCount++;document.getElementById('stat-found').textContent=foundCount;document.getElementById('grid').appendChild(renderCard(result.ip,result.info,result.sensors));}}
  ...
  if(foundCount===0){document.getElementById('grid').innerHTML='<div class="empty"><div class="icon">🔍</div><p>No ESP32 Smart Monitor units found on <strong>'+subnet+'.'+from+'–'+to+'</strong>.<br>Make sure all units are on the same WiFi and try adjusting the subnet or timeout.</p></div>';setStatus('No devices found','');}
  else{setStatus('Found '+foundCount+' device'+(foundCount>1?'s':''),'done');}
}
function stopScan(){stopFlag=true;clearInterval(statsTimer);setStatus('Stopped','');document.getElementById('scan-btn').disabled=false;document.getElementById('stop-btn').disabled=true;scanning=false;}
</script>
</body>
</html>
)rawliteral";
